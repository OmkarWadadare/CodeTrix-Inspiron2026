...
from app.models.schemas import CreateOrgRequest, JoinOrgRequest, InviteCodeRequest, RemoveMemberRequest
...
from app.db.database import (create_organization, get_organization_by_email, get_org_members,
                              add_org_member, remove_org_member, generate_invite_code,
                              validate_invite_code, join_organization, get_org_invite_codes,
...
async def translate_segments(request: TranslationRequest):
    ...
    try:
        ...
        for i, segment in enumerate(request.segments):
            ...
            if match["type"] in ("FUZZY", "EXACT") and match["confidence"] >= 0.95:
                results.append({
                    "source": segment,
        ...
        if segments_to_ai:
            ...
            if ai_translations and len(ai_translations) == len(segments_to_ai):
                ...
                for idx in ai_indices:
                    results[idx] = {"source": request.segments[idx], "translation": "Translation Error", "status": "ERROR"}
...
async def upload_document(file: UploadFile = File(...)):
    ...
    try:
        ...
        if not result.get("segments"):
            raise HTTPException(400, "No text content could be extracted from this file")
...
async def translate_document(request: DocumentTranslationRequest):
    ...
    if not os.path.exists(json_path):
        raise HTTPException(404, f"Extracted JSON not found. Please re-upload the document.")
    ...
    translated_texts = translate_batch_with_groq(
        segment_texts, request.target_lang, request.tone, glossary, request.source_lang
    )

    if not translated_texts:
        raise HTTPException(500, "Translation returned empty. Check GROQ_API_KEY.")

    translations = dict(zip(segment_ids, translated_texts))
    ...
    for seg_id, orig_text in zip(segment_ids, segment_texts):
        translated_text = translations.get(seg_id, "")
        add_to_tm(orig_text, translated_text, request.target_lang)
        log_action(orig_text, "DOCUMENT_TRANSLATION", "", translated_text)

    return {
        "download_url": f"/api/download/{request.session_id}/{ext}",
        "translated_text": translated_text_combined,
...
async def create_org(request: CreateOrgRequest):
    org_id = create_organization(request.name, request.owner_email)
    return {"org_id": org_id, "name": request.name}

@router.get("/org/me")
async def get_my_org(email: str):
    org = get_organization_by_email(email)
    if not org:
        return {"org": None}
    members = get_org_members(org["id"])
    codes = get_org_invite_codes(org["id"])
    return {"org": {**org, "members": members, "codes": codes}}

@router.post("/org/invite-code")
async def create_invite_code(request: InviteCodeRequest):
    code = generate_invite_code(request.org_id, request.created_by, request.expires_hours)
    return {"code": code}

@router.post("/org/join")
async def join_org(request: JoinOrgRequest):
    result = join_organization(request.code, request.email)
    if "error" in result:
...
@router.post("/org/remove-member")
async def remove_member(request: RemoveMemberRequest):
    owner = query_db('SELECT * FROM org_members WHERE org_id=? AND email=? AND role="owner"', (request.org_id, request.requester_email), one=True)
    if not owner:
        raise HTTPException(403, "Only the owner can remove members")
    remove_org_member(request.org_id, request.email)
    return {"status": "REMOVED"}

@router.post("/org/deactivate-code")
async def deactivate_code(code_id: str):
    deactivate_invite_code(code_id)
